// src/shared/calendar/service/doxy.service.ts
...
export class DoxyService implements IVideoProvider {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        this.clinicSubdomain = this.configService.get<string>('doxy.clinicSubdomain') || '';
        this.defaultProviderRoom = this.configService.get<string>(
            'doxy.defaultProviderRoom',
        ) || '';
    ...
    async createRoom(params: CreateRoomParams): Promise<VideoRoomResponse> {
        try {
            ...
            const doctorRoom = params.metadata?.doctorRoom || this.defaultProviderRoom;
    ...
    async getRoom(roomId: string): Promise<VideoRoomResponse> {
        this.logger.warn('Doxy.me getRoom called - limited functionality without API');
   ...
   // Generate a Doxy.me room for a specific doctor
    ...
    // Validate Doxy.me webhook signature (if webhooks are enabled)
    verifyWebhookSignature(payload: string, signature: string): boolean {
        const webhookSecret = this.configService.get<string>('doxy.webhookSecret');

        if (!webhookSecret) {
            this.logger.warn('Doxy webhook secret not configured');
            return false;
        }
