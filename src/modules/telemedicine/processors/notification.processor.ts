...
export class NotificationProcessor {
    ...
    async handleCancellationEmail(job: Job) {
        try {
            const { bookingId, refundAmount, patientEmail, patientName, doctorEmail, doctorName, type } = job.data;
    ...
    async handleReviewRequest(job: Job) {
        try {
            const { bookingId, patientId, doctorId, patientEmail, patientName } = job.data;
